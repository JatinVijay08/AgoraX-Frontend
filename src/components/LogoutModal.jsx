...
export default function LogoutModal({ isOpen, onClose, onConfirm }) {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 atelier-overlay" style={{ justifyContent: 'center', alignItems: 'flex-start', paddingTop: '20vh' }}>
            <div className="atelier-panel max-w-sm w-full p-8 relative" >
                <button
                    onClick={onClose}
                ...
                <div>
                    <div className="w-14 h-14 rounded-sm flex items-center justify-center mb-6" style={{ background: 'rgba(255, 180, 171, 0.1)' }}>
                        <span className="material-symbols-outlined text-[24px] text-error">logout</span>
                    </div>

                    <h3 className="text-[1.5rem] font-[700] text-on-surface tracking-tight mb-2" style={{ fontFamily: 'var(--font-cinzel)' }}>
                        Depart the Agora?
                    </h3>
                    <p className="text-[14px] text-on-surface-variant leading-[1.7] mb-8">
                        You'll need to return to the public square to share your ideas and debate.
                    </p>

                    <div className="flex flex-col gap-3">
                        <button
                            onClick={onConfirm}
                            className="w-full py-3.5 text-[15px] font-[800] cursor-pointer text-canvas rounded-sm"
                            style={{ background: 'linear-gradient(to right, #ff8a80, #ffb4ab)' }}
                        >
                            Sign Out
                        </button>
